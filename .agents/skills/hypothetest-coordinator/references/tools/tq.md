...
For scalar shell assignments use `-r` to avoid TOON sequence framing. Use `-e`/`--exit-status` when a null or false extraction must fail, and verify that the installed `tq --help` exposes this option during readiness. Use the installed filter set; the sampling examples retain fractional averages and do not require a rounding builtin.
